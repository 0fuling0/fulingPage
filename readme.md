...
*   **阶段4 (已完成 2026-09-12):** 构建多套可随时切换的主题，并同步完成一组体验修复与性能优化。
    ...
    *   **修复与优化：** 暗色切换按钮图标实时更新；移动端断点圆角改为主题变量（`--card-radius-mobile` 等）保证单列/多列一致；悬停光效统一走 `--glow-color`；`scrollbar-gutter: stable` 常驻滚动条槽位，内容高度变化时主元素不再横移；评论（Twikoo）与音乐播放器（APlayer/Meting）脚本按配置按需加载，对应卡片未启用时不下载；背景图预加载使用与展示相同的地址（消除 API 图二次下载）+ `img.decode()` 预解码 + 图层合成器提升（`will-change`），切换动画附 Ken Burns 缓推，整行源码流动动画（`ANIMATIONS`）；PSI 对比度/标签/可爬取链接等无障碍与 SEO 修复；背景图压缩至 1920 宽（体积 -31%）、雨滴/代码流动降为 30fps + 帧间隔补偿、`prefers-reduced-motion` 时停用持续动画、文本选中色跟随主题。

...
*   **阶段4 (已完成 2026-09-12):** 构建多套可随时切换的主题，并同步完成一组体验修复与性能优化。
    ...
    *   **修复与优化：** 暗色切换按钮图标实时更新；移动端断点圆角改为主题变量（`--card-radius-mobile` 等）保证单列/多列一致；悬停光效统一走 `--glow-color`；`scrollbar-gutter: stable` 常驻滚动条槽位，内容高度变化时主元素不再横移；评论（Twikoo）与音乐播放器（APlayer/Meting）脚本按配置按需加载，对应卡片未启用时不下载；PSI 优化——预加载 `config.json`/`nav.json`、背景图低优先级加载、页脚动作链接改用 `button`（可爬取）、搜索框/按钮补可访问名、各主题不透明背景下的文字对比度达 WCAG AA（按钮渐变加深、muted 色与占位符/透明度调整）。
